...
camera = document.getElementById("camera"); Webcam.attach('#camera');
 function take_snapshot() { Webcam.snap(function(data_uri) 
  ...
  console.log('ml5 version:', ml5.version);
   classifier = ml5.imageClassifier('https://teachablemachine.withgoogle.com/models/5QFkoqWad/model.json',modelLoaded);
 ...
 function speak(){ var synth = window.speechSynthesis;
  ...
  function gotResult(error, results) { 
      if (error) { console.error(error);
           ...
           console.log(results); document.getElementById("result_emotion_name").innerHTML = results[0].label; 
